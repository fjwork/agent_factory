...
class AuthenticatedRequestHandler(DefaultRequestHandler):
    ...
    async def _store_oauth_in_session_state(self, user_context: Dict[str, Any], body: bytes) -> None:
        ...
        try:
            ...
            # Store auth context for the before_agent_callback to inject into session
            # This is much simpler than trying to pre-create sessions
            oauth_state = {
            ...
            # Store in global registry for before_agent_callback to access
            await self._store_oauth_in_global_registry(user_context)

            # Also store the oauth_state for callback injection
            if not hasattr(self.__class__, '_pending_auth_injection'):
                self.__class__._pending_auth_injection = {}

            # Use user_id as key for the callback to find the right auth context
            user_id = user_context.get("user_id")
            if user_id:
                self.__class__._pending_auth_injection[user_id] = oauth_state
                logger.info(f"Stored auth context for callback injection, user: {user_id}")
            else:
                logger.warning("No user_id in auth context, callback injection may not work")
